...
public class StaticShooterSystem extends SubsystemBase {
    ...
    private final SparkMax shooterStabilisationMotor;
    private final SparkMax feederMotor;
    private final Counter counter;

    //travel time calc variables
    private final double a = -9.81;//gravity
    private double b = 0;
    private double c = 0;
...
//    public double ballVelocityrpm;
    ...
    public StaticShooterSystem() {
        ...
        shooterStabilisationMotor = new SparkMax(RobotMap.SHOOTER_FEEDER_STABILIZER_MOTOR, SparkLowLevel.MotorType.kBrushless);
        ...
        shooterStabilisationMotor.configure(configFeedStabilizer, ResetMode.kNoResetSafeParameters, PersistMode.kNoPersistParameters);

        shooterEncoder = shooterMotor.getEncoder();
        shooterStabilisationEncoder = shooterStabilisationMotor.getEncoder();
    }
    ...
    public void setShootSpeed(double reqRPM) {
        ...
        shooterStabilisationMotor.getClosedLoopController().setSetpoint(reqRPM * (RobotMap.SHOOTER_WHEEL_RADIUS_METERS / RobotMap.SHOOTER_FEEDER_STABILIZER_WHEEL_RADIUS_METERS), SparkBase.ControlType.kVelocity);
    }
    ...
    public void stopShooterAndFeeder() {
        ...
        shooterStabilisationMotor.stopMotor();
    }
...
//    }
    ...
    public void setConfig(SparkMaxConfig config) {
        shooterStabilisationMotor.configure(config, ResetMode.kNoResetSafeParameters, PersistMode.kPersistParameters);
    }

    public ClosedLoopSlot getSlot() {
        return shooterStabilisationMotor.getClosedLoopController().getSelectedSlot();
    }

    public double getSetPoint() {
        return shooterStabilisationMotor.getClosedLoopController().getSetpoint();
    }

    public void setSetPoint(double setPoint) {
        shooterStabilisationMotor.getClosedLoopController().setSetpoint(setPoint, SparkBase.ControlType.kVelocity);
    }

    public void setPower(double power) {
        shooterStabilisationMotor.set(power);
